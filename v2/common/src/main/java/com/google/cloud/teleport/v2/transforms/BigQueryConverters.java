...
public class BigQueryConverters {
  ...
  /**
   * The {@link FailsafePubSubEventJsonToTableRow} transform converts JSON strings to {@link
   * TableRow} objects by extracting each's `data` property and mapping each of its keys to a
   * column. The transform accepts a {@link FailsafeElement} object so the original payload of the
   * incoming record can be maintained across multiple series of transforms.
   */
  @AutoValue
